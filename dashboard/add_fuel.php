...
<head>
    ...
    <title>New Fuel Details | Vehicle Information Management System</title>
    ...
    <script>
        function success() {
            Swal.fire({
                position: 'center',
                icon: 'success',
                title: 'Fuel details has been created Successfully',
            ...
            <div class="grid main">
                ...
                <div class="top_card">
                    <div class="bread-cumb">
                        <a href="index.php">Dashboard</a> > <a href="fuel.php" id="sec">Fuel</a> > <span>New Fuel Details</span>
                    </div>
                </div>
                <div class="bottom-card">
                    <form action="" method="post" enctype="multipart/form-data">
                    <p style="border-bottom: 1px #eee solid; padding-bottom:5px;margin-bottom:5px">Fuel Registration</p>
                        <div class="input_row">
                            <div class="inputBox">
                                <span>Fuel in Litres</span>
                                <input type="text" name="litres" value="<?=$litres ?? ''?>" placeholder="Eg. 12.5">
                                <p><?=$errors['litres'] ?? ''?></p>
                            </div>
                            <div class="inputBox">
                                <span>Fuel Cost</span>
                                <input type="text" name="cost" value="<?=$cost ?? ''?>" placeholder="Eg. 120000">
                                <p><?=$errors['cost'] ?? ''?></p>
                            </div>
                            <div class="inputBox">
                                <span>Purchased on</span>
                                <input type="date" name="purchased" value="<?=$purchased ?? ''?>">
                                <p><?=$errors['purchased'] ?? ''?></p>
                            </div>
                        </div>
                        <div class="input_row" style="justify-content:left;" style="display: flex;align-items:center">
                            <div class="inputBox">
                                ...
                                <select name="vehicleID" id="vehicleID">
                                    ...
                                    <?php
                                        try {
                                            ...
                                            foreach($rows as $row): ?>
                                                <option value="<?=$row['vehicleID']?>"><?=$row['registration_no']?></option>
                                            <?php endforeach;
                                        } catch (PDOException $e) {
                                            echo $e->getMessage();
                                        }
                            ...
                            <div class="inputBox" style="margin-left: 22px;">
                                <span style="display: block">Upload Receipt Image</span>
                                <input type="file" name="image" id="fileInput" style="visibility: hidden; position:absolute">
                                <label class="upload" for="fileInput" style="cursor: pointer; display:inline-block; font-size: small; background: var(--orange); padding: 7px 20px; margin-top:2px !important; border-radius: 5px;">Choose Image</label>
                                <p style="margin:0"><?=$errors['image'] ?? ''?></p>
                            </div>
