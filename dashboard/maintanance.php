...
<head>
    ...
    <title>Maintenance | Vehicle Information Management System</title>
            ...
            <div class="grid main">
                ...
                <div class="top_card">
                    <div class="bread-cumb">
                        <a href="index.php">Dashboard</a> > <span>Maintenance</span>
                    </div>
                    <a href="add_maintenance.php" class="add-btn"><i class="bx bxs-car-mechanic"></i>Add Maintenance</a>
                </div>
                <div class="top_card" style="align-items: center;">
                    <p style="color:black">Maintenance History </p>
                    <input type="text" name="searchInput" id="searchInput" placeholder="Search Maintanance history..">
                </div>
                <div class="bottom-card">
                    <table id="myTable">
                        ...
                        <?php 
                            try {
                                require('../db/pdo.php');
                                $stmt = $dbCnx->prepare('SELECT maintenance.*, vehicle.registration_no FROM maintenance INNER JOIN vehicle ON maintenance.vehicleID = vehicle.vehicleID ORDER BY mainID DESC');
                                $stmt->execute();
                                $rows = $stmt->fetchAll();
                                $sn = 1;
                                foreach($rows as $row):
                                ...
                                ?>
                                    <tr id="trow">
                                        ...
                                        <td><?=date('jS F Y', strtotime($row['last_maintenance']))?></td>
                                        <td><?=date('jS F Y', strtotime($row['next_maintenance']))?></td>
                                        <!-- <td><?=$next_m_days?></td> -->
                                        <!-- <td><?php //echo $row['battery_check']?></td> -->
                                        <td><a href="view_maintenance.php?id=<?=$row['mainID']?>"><i class="bx bx-file icon res"></i></a><a href="edit_maintenance.php?id=<?=$row['mainID']?>"><i class="bx bx-edit icon update"></i></a><a href="delete_maintenance.php?id=<?=$row['mainID']?>"><i class="bx bx-trash icon del"></i></a></td>
