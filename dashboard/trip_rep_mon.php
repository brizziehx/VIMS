            ...
            <tbody>
              <?php 
                try {
                  ...
                  $stmt = $dbCnx->prepare("SELECT trips.*, vehicle.* FROM trips INNER JOIN vehicle ON trips.vehicleID = vehicle.vehicleID  INNER JOIN routes ON vehicle.routeID = routes.routeID  WHERE vehicle.routeID = :routeID AND month(trips.created_at) = :month");
                  $stmt->bindValue(':routeID', $_GET['routeID'], PDO::PARAM_INT);
