            ...
            <tbody>
              <?php 
                try {
                  ...
                  $stmt = $dbCnx->prepare("SELECT maintenance.*, vehicle.* FROM maintenance INNER JOIN vehicle ON maintenance.vehicleID = vehicle.vehicleID AND year(maintenance.created_at) = year(curdate())");
                  $stmt->execute();
