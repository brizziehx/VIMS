      ...
      <div class="invoice print">
        ...
        <div class="invoice-body">
            ...
            <tbody>
              <?php 
                try {
                  ...
                  $stmt = $dbCnx->prepare("SELECT oil.*, vehicle.* FROM oil INNER JOIN vehicle ON oil.vehicleID = vehicle.vehicleID");
                  $stmt->execute();
          ...
          <?php 
                try {
                  require('../db/pdo.php');
                  $query = $dbCnx->query("SELECT oil_type, COUNT(*) AS count FROM oil GROUP BY oil_type ORDER BY count DESC LIMIT 1");
                  if($query->rowCount() > 0):
                    $row = $query->fetch();  
                    if($row['count'] > 1):
                      $used = 'times';
