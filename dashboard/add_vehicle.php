<?php
    ...
    $regno = $make = $type = $year = $chassisno = $model = $cc = $fuel = $engine_no = $current_km = "";
    if(isset($_POST['submit'])) {
        ...
        $routeID = $_POST['route'] ?? '';
        $current_km = trim($_POST['current_km']);
        ...
        if(empty($regno)) {
            ...
            if(!preg_match("/^[T][0-9']{3} [A-Z]{3}$/", $regno)) {
                ...
                try {
                    require('../db/pdo.php');
                    $stmt = $dbCnx->prepare('SELECT * FROM vehicle WHERE registration_no = :rg');
                    $stmt->bindValue(':rg', $regno);
                    $stmt->execute();
        ...
        if(empty($current_km)) {
            $errors['current_km'] = 'Current kilometers are required';
        } else {
            if(!preg_match("/^[0-9]{1,}$/", $current_km)) {
                $errors['current_km'] = "Current kilometers are not valid";
            }
        }
        ...
        if(empty($chassisno)) {
            ...
            if(!preg_match("/^[A-Z0-9]{17}$/", $chassisno)) {
                ...
                try {
                    require('../db/pdo.php');
                    $stmt = $dbCnx->prepare('SELECT * FROM vehicle WHERE chassis_no = :cn');
        ...
        if(empty($engine_no)) {
            ...
            if(!preg_match("/^[A-Z0-9]{11,}$/", $engine_no)) {
                ...
                try {
                    require('../db/pdo.php');
                    $stmt = $dbCnx->prepare('SELECT * FROM vehicle WHERE engine_no = :en');
        ...
        if(!array_filter($errors)) {
            try {
                ...
                $stmt = $dbCnx->prepare('INSERT INTO vehicle (model, type, year, make, chassis_no, registration_no, engine_no, cc, transmission, fuel, created_at, driverID, routeID) VALUES (:model, :type, :year, :make, :chassis_no, :registration_no, :engine_no, :cc, :transmission, :fuel, :created_at, :driverID, :routeID)');
                ...
                if($stmt->execute()) {
                    $regno = $make = $type = $year = $auto = $manual = $gas = $diz = $chassisno = $model = $cc = $fuel = $engine_no = "";
...
<head>
    ...
    <title>New vehicle | Vehicle Information Management System</title>
            ...
            <div class="grid main">
                ...
                <div class="top_card">
                    <div class="bread-cumb">
                        <a href="index.php">Dashboard</a> > <a href="vehicle.php" id="sec">vehicles</a> > <span>New Vehicle</span>
                    </div>
                </div>
                <div class="bottom-card">
                    <form action="" method="post" enctype="multipart/form-data">
                        <p style="border-bottom: 1px #eee solid; padding-bottom:5px;margin-bottom:5px">Vehicle Registration</p>
                        <div class="input_row">
                            <div class="inputBox">
                                ...
                                <select name="driver">
                                    ...
                                    <?php
                                        try {
                                            ...
                                            $stmt = $dbCnx->prepare('SELECT * FROM users WHERE usertype = :usertype AND NOT EXISTS(SELECT driverID FROM vehicle WHERE vehicle.driverID = users.userID)');
                        ...
                        <div class="input_row">
                            <div class="inputBox">
                                <input type="submit" name="submit" value="Register vehicle"> 
                            </div>
                            <?=$errors['error'] ?? ''?>
                            <?=$errors['success'] ?? ''?>
                        </div>
