      ...
      <div class="invoice print">
        ...
        <div class="invoice-body">
            ...
            <tbody>
              <?php 
                try {
                  ...
                  $stmt = $dbCnx->prepare("SELECT * FROM vehicle  WHERE  created_at BETWEEN :start_date AND :end_date");
          ...
          <?php 
                try {
                  require('../db/pdo.php');
                  $query = $dbCnx->query("SELECT * FROM vehicle WHERE created_at BETWEEN '{$_GET['sdate']}' AND '{$_GET['edate']}'");
                  $query2 = $dbCnx->query("SELECT * FROM vehicle WHERE created_at BETWEEN '{$_GET['sdate']}' AND '{$_GET['edate']}' AND status = 'active'");
                  $query3 = $dbCnx->query("SELECT * FROM vehicle WHERE created_at BETWEEN '{$_GET['sdate']}' AND '{$_GET['edate']}' AND status = 'inactive'");
                  if($query->rowCount() > 0):
                    $row = $query->fetch();  
