<?php
    ...
    if(isset($_POST['submit'])) {
        ...
        if(!array_filter($errors)) {
            try {
                ...
                $stmt = $dbCnx->prepare('UPDATE maintenance SET vehicleID = :vehicleID, last_maintenance = :last_maintenance, next_maintenance = :next_maintenance, actions = :actions, updated_at = :updated_at WHERE mainID = :mainID');
                ...
                $stmt->bindValue(':mainID', $_REQUEST['id'], PDO::PARAM_INT);
    ...
    try {
        ...
        $stmt2 = $dbCnx->prepare('SELECT * FROM maintenance WHERE mainID = :mainID');
...
<head>
    ...
    <title>Edit Maintanance | Vehicle Information Management System</title>
            ...
            <div class="grid main">
                ...
                <div class="top_card">
                    <div class="bread-cumb">
                        <a href="index.php">Dashboard</a> > <a href="maintanance.php" id="sec">Maintenance</a> > <span>Edit Maintenance</span>
                    </div>
                </div>
                <div class="bottom-card">
                    <form action="" method="post" enctype="multipart/form-data">
                    <p style="border-bottom: 1px #eee solid; padding-bottom:5px;margin-bottom:5px">Edit Maintanance</p>
                        <div class="input_row">
                            <div class="inputBox">
                                <span>Last Maintenance</span>
                                <input type="date" name="last_m" value="<?=$row2['last_maintenance'] ?? ''?>">
                                <p><?=$errors['last_m'] ?? ''?></p>
                            </div>
                            <div class="inputBox">
                                <span>Next Maintenance</span>
                                <input type="date" name="next_m" value="<?=$row2['next_maintenance'] ?? ''?>">
                                <p><?=$errors['next_m'] ?? ''?></p>
                            </div>
                            <div class="inputBox">
