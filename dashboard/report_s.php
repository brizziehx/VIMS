      ...
      <div class="invoice print">
        ...
        <div class="invoice-head">
          <div class="head client-data">
            ...
            <p>Service Report</p>
            <p>From <?=date('M, jS Y', strtotime($_GET['sdate']))?> To <?=date('M, jS Y', strtotime($_GET['edate']))?></p>
          </div>
        </div>
        <!-- invoice body-->
        <div class="invoice-body">
            ...
            <tbody>
              <?php 
                try {
                  ...
                  $stmt = $dbCnx->prepare("SELECT oil.*, vehicle.* FROM oil INNER JOIN vehicle ON oil.vehicleID = vehicle.vehicleID WHERE oil.vehicleID = :vehicleID AND oil.created_at BETWEEN :start_date AND :end_date");
          ...
          <?php 
                try {
                  require('../db/pdo.php');
                  $query = $dbCnx->query("SELECT oil_type, COUNT(*) AS count FROM oil WHERE vehicleID = {$vehicleID} AND created_at BETWEEN '{$start_date}' AND '{$end_date}' GROUP BY oil_type ORDER BY count DESC LIMIT 1");
                  if($query->rowCount() > 0):
                    $row = $query->fetch();  
                    if($row['count'] > 1):
                      $used = 'times';
